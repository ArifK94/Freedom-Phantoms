...
AOrderIcon::AOrderIcon()
{
	...
	Shape->AttachToComponent(Head, FAttachmentTransformRules::KeepRelativeTransform);
	Shape->SetCollisionProfileName(TEXT("NoCollision"));
	Shape->CanCharacterStepUpOn = ECB_No;
...
void AOrderIcon::ShowIcon(UMaterialInstance* IconMaterial, UMaterialInstance* ShapeMaterial)
{
	Head->SetMaterial(0, IconMaterial);
	Shape->SetMaterial(0, ShapeMaterial);
	ShowIcon();
}
